...
Для более углубленного знакомства с механизмом планирования потоков в Windows рекомендую изучить [документацию на сайте microsoft.com](https://docs.microsoft.com/en-us/windows/win32/procthread/scheduling).

# 6.1 Роль системного таймера (system clock) <a name="os_timer"></a>

Как уже было сказано в разделе 1.3, системный планировщик автоматически выполняет действия, необходимые для перевода потока в спящий режим, как только поток истратит выделенный ему квант времен (time slice), либо процессорное время потребуется потоку с более высоким приоритетом. Также поток может самостоятельно уйти в спящий режим, в тех случаях, если он выполнил свою работу, либо перешёл в режим ожидания события. Каждый раз, когда поток переходит в спящий режим, происходит сохранение его контекста (сохраняется текущее состояние регистров процессорного ядра, на котором поток выполняется). При переводе потока в рабочий режим выполняется обратное действие – восстановление контекста потока (загрузка ранее сохраненных значений в регистры процессорного ядра). 
...
а) без поступления сигнала прерывания от системного таймера: например, другой поток произвёл вызов функции ОС, которая приостановила его работу и переключила контекст на поток, находившийся в спящем режиме (таких функций много, в том числе `Sleep`, `SwitchToThread`, `SetEvent` и др.);
б) в результате поступления сигнала прерывания от системного таймера: работа потока, который выполняется на ядре процессора, прерывается, далее анализируется информация, подготовленная системным планировщиком, а затем либо работа потока на этом же ядре возобновляется, либо производится переключение контекста на этом же ядре на другой поток в соответствии с указаниями, поступившими от планировщика.
...
1) системный таймер подаёт сигнал прерывания на все ядра процессора;
2) приостанавливается работа всех выполняемых потоков на всех ядрах процессора;
3) осуществляется запуск системного планировщика на одном из ядер процессора (Sheduler Core);
4) на других ядрах процессора производится обработка указаний планировщика, которые были подготовлены при предыдущем срабатывании системного таймера;
5) планировщик анализирует состояние всех потоков в системе и принимает решение о необходимости запуска или приостановки того или иного потока (он сохраняет необходимую информацию в память ядра ОС);
6) на ядре Sheduler Core производится обработка указаний планировщика (скорее всего, актуальных);
...
Очевидно, что чем больше потоков, тем хуже производительность системы. Я предполагаю, что при каждом запуске системного планировщика он производит анализ всех запущенных потоков, независимо от их состояния. Чем больше потоков, тем больше действий будет выполнять планировщик.
